...
local floor, clamp, random, sin, cos, pi, abs, max, min =
    math.floor, math.clamp, math.random, math.sin, math.cos, math.pi, math.abs, math.max, math.min
...
local theme_presets = {
    Nebula = {
        bg_dark = rgb(18,18,24), bg_mid = rgb(24,24,32), bg_light = rgb(30,30,40),
    ...
    Azure = {
        bg_dark = rgb(14,20,32), bg_mid = rgb(20,28,44), bg_light = rgb(28,38,58),
    ...
    Crimson = {
        bg_dark = rgb(22,12,14), bg_mid = rgb(30,16,20), bg_light = rgb(40,22,26),
    ...
    Emerald = {
        bg_dark = rgb(12,22,18), bg_mid = rgb(16,30,24), bg_light = rgb(22,40,32),
    ...
    Monochrome = {
        bg_dark = rgb(14,14,14), bg_mid = rgb(20,20,20), bg_light = rgb(28,28,28),
    ...
    Midnight = {
        bg_dark = rgb(8,8,14), bg_mid = rgb(12,12,20), bg_light = rgb(18,18,28),
    ...
    Sunset = {
        bg_dark = rgb(26,16,20), bg_mid = rgb(34,20,26), bg_light = rgb(46,28,34),
    ...
    Ocean = {
        bg_dark = rgb(10,20,28), bg_mid = rgb(14,26,36), bg_light = rgb(20,36,50),
    ...
    Rose = {
        bg_dark = rgb(24,14,20), bg_mid = rgb(32,18,26), bg_light = rgb(44,26,36),
    ...
    Forest = {
        bg_dark = rgb(14,20,12), bg_mid = rgb(20,28,16), bg_light = rgb(28,38,22),
...
local function tw_spring(obj, props, t)
    tween_svc:Create(
        obj,
        TweenInfo.new(t or 0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
        props
    ):Play()
end
...
local popup_root = mk("ScreenGui", {
    Name = "NebulaPopups", IgnoreGuiInset = true,
    DisplayOrder = 999, ResetOnSpawn = false
})
parent_gui(popup_root)

local notif_gui = mk("ScreenGui", {
    Name = "NebulaNotifs", IgnoreGuiInset = true,
    DisplayOrder = 998, ResetOnSpawn = false
})
parent_gui(notif_gui)

local overlay_gui = mk("ScreenGui", {
    Name = "NebulaOverlay", IgnoreGuiInset = true,
    DisplayOrder = 950, ResetOnSpawn = false
})
...
local function ensure_blur()
    ...
    for _, v in ipairs(lit:GetChildren()) do
        if v.Name == "NebulaBlur" or v.Name == "NebulaCC" or v.Name == "NebulaDOF" then
            v:Destroy()
        end
    end
    local b = Instance.new("BlurEffect")
    b.Name = "NebulaBlur"; b.Size = 0; b.Parent = lit
    local cc = Instance.new("ColorCorrectionEffect")
    cc.Name = "NebulaCC"; cc.Saturation = 0; cc.Brightness = 0
    cc.Contrast = 0; cc.TintColor = Color3.new(1,1,1); cc.Parent = lit
    local dof = Instance.new("DepthOfFieldEffect")
    dof.Name = "NebulaDOF"; dof.FarIntensity = 0; dof.NearIntensity = 0
    dof.FocusDistance = 20; dof.InFocusRadius = 10; dof.Parent = lit
    nebula._color_correction = cc; nebula._dof = dof
    return b
end

local function set_blur_amount(target_size, duration)
    duration = duration or 0.35
    local b   = nebula._blur
    local cc  = nebula._color_correction
    local dof = nebula._dof
    if not b or not cc or not dof then return end
    local start_b    = b.Size
    local start_s    = cc.Saturation
    local start_far  = dof.FarIntensity
    local start_near = dof.NearIntensity
    local on          = target_size > 0
    local target_s    = on and -0.4 or 0
    local target_far  = on and 0.6  or 0
    local target_near = on and 0.4  or 0
    local steps = max(1, floor(duration / 0.016))
    task.spawn(function()
        for i = 1, steps do
            local t = ease_out_cubic(i / steps)
            pcall(function()
                b.Size            = start_b    + (target_size  - start_b)    * t
                cc.Saturation     = start_s    + (target_s    - start_s)    * t
                dof.FarIntensity  = start_far  + (target_far  - start_far)  * t
                dof.NearIntensity = start_near + (target_near - start_near) * t
            end)
            task.wait(0.016)
        end
        pcall(function()
            b.Size = target_size; cc.Saturation = target_s
            dof.FarIntensity = target_far; dof.NearIntensity = target_near
        end)
    end)
end

local function make_vignette(parent)
    local v = mk("Frame", {
        Parent = parent, BackgroundColor3 = Color3.new(0,0,0),
        BackgroundTransparency = 1, BorderSizePixel = 0,
        Size = dim2(1,0,1,0), ZIndex = 899, ClipsDescendants = false,
    })
    mk("UIGradient", {
        Parent = v, Rotation = 90,
        Transparency = NumberSequence.new{
            NumberSequenceKeypoint.new(0,   0.05), NumberSequenceKeypoint.new(0.3,  0.85),
            NumberSequenceKeypoint.new(0.7,  0.85), NumberSequenceKeypoint.new(1,   0.05),
        },
    })
    return v
end

local function make_romania_tag(parent)
    local holder = mk("Frame", {
        Parent = parent, BackgroundTransparency = 1, BorderSizePixel = 0,
        Position = dim2(1, -250, 1, -38), Size = dim2(0, 238, 0, 26),
        ZIndex = 1000, Visible = false,
    })
    local lbl = mk("TextLabel", {
        Parent = holder, BackgroundTransparency = 1, Size = dim2(1, 0, 1, 0),
        Font = Enum.Font.GothamBold, Text = "made in Romania :p",
        TextSize = 15, TextColor3 = Color3.new(1,1,1),
        TextStrokeTransparency = 0.6, TextStrokeColor3 = Color3.new(0,0,0),
        TextXAlignment = Enum.TextXAlignment.Right,
        TextYAlignment = Enum.TextYAlignment.Center, ZIndex = 1001,
    })
    local grad = mk("UIGradient", {
        Parent = lbl, Rotation = 0,
        Color = ColorSequence.new{
            ColorSequenceKeypoint.new(0,    Color3.new(0,0,0)),
            ColorSequenceKeypoint.new(0.45, theme.accent),
            ColorSequenceKeypoint.new(1,    theme.text_accent),
        },
    })
    local function refresh_gradient()
    ...
    insert(theme_change_callbacks, refresh_gradient)
    nebula._romania = { holder = holder, label = lbl, gradient = grad, refresh = refresh_gradient }
    return nebula._romania
end

local function make_shell_shade(shell)
    local shade = mk("Frame", {
        Parent = shell, BackgroundColor3 = Color3.new(0,0,0),
        BackgroundTransparency = 1, BorderSizePixel = 0,
        Size = dim2(1,0,1,0), ZIndex = 5, ClipsDescendants = true,
    })
    mk("UICorner", { Parent = shade, CornerRadius = dim(0,6) })
    local function shadow(sz, pos, rot)
        local f = mk("Frame", {
            Parent = shade, BackgroundColor3 = Color3.new(0,0,0),
            BackgroundTransparency = 0, BorderSizePixel = 0,
            Size = sz, Position = pos, ZIndex = 6,
        })
        mk("UIGradient", {
            Parent = f, Rotation = rot,
            Transparency = NumberSequence.new{
                NumberSequenceKeypoint.new(0, 0.55), NumberSequenceKeypoint.new(1, 1),
            },
        })
    end
    shadow(dim2(1,0,0,80),   dim2(0,0,0,0),    90)
    shadow(dim2(1,0,0,80),   dim2(0,0,1,-80),  270)
    shadow(dim2(0,60,1,0),   dim2(0,0,0,0),    0)
    shadow(dim2(0,60,1,0),   dim2(1,-60,0,0),  180)
    return shade
end

local SNOW_LAYERS = {
    { count = 60, size_min = 2, size_max = 4, speed_min = 0.3, speed_max = 0.9, alpha_min = 0.55, alpha_max = 0.75, sway_amp = 0.5, pulse_speed = 1.2 },
    { count = 50, size_min = 3, size_max = 6, speed_min = 0.8, speed_max = 1.6, alpha_min = 0.35, alpha_max = 0.60, sway_amp = 0.8, pulse_speed = 0.8 },
    { count = 30, size_min = 5, size_max = 9, speed_min = 1.4, speed_max = 2.4, alpha_min = 0.20, alpha_max = 0.45, sway_amp = 1.2, pulse_speed = 0.5 },
}

local function make_snow(parent)
    local holder = mk("Frame", {
        Parent = parent, BackgroundTransparency = 1, BorderSizePixel = 0,
        Size = dim2(1,0,1,0), ZIndex = 900, ClipsDescendants = true,
    })
    local flakes = {}
    local vx = camera.ViewportSize.X
    local vy = camera.ViewportSize.Y
    for layer_idx, layer in ipairs(SNOW_LAYERS) do
        for _ = 1, layer.count do
            local sz    = layer.size_min + random() * (layer.size_max - layer.size_min)
            local alpha = layer.alpha_min + random() * (layer.alpha_max - layer.alpha_min)
            local f = mk("Frame", {
                Parent = holder, BackgroundColor3 = theme.text_accent,
                BorderSizePixel = 0, Size = dim2(0, sz, 0, sz),
                Position = dim2(0, random(0, vx), 0, random(-vy, vy)),
                BackgroundTransparency = 1 - alpha, ZIndex = 900 + layer_idx,
            })
            mk("UICorner", { Parent = f, CornerRadius = dim(1,0) })
            insert(flakes, {
                obj = f, speed = layer.speed_min + random() * (layer.speed_max - layer.speed_min),
                drift = (random() - 0.5) * 0.5, sway = random() * pi * 2,
                sway_amp = layer.sway_amp * (0.7 + random() * 0.6),
                pulse_phase = random() * pi * 2,
                pulse_speed = layer.pulse_speed * (0.8 + random() * 0.4),
                base_alpha = alpha, layer = layer_idx, wind_offset = 0,
            })
        end
    end
    return holder, flakes
end

local _wind_time    = 0
local _wind_target  = 0
local _wind_current = 0
local _wind_timer   = 0

local function update_snow(dt)
    local holder = nebula._snow
    local flakes = nebula._snow_flakes
    if not holder or not holder.Parent then return end
    _wind_timer = _wind_timer + dt
    if _wind_timer > 4 + random() * 4 then
        _wind_timer  = 0
        _wind_target = (random() - 0.5) * 2.5
    end
    local wind_lerp = 1 - (0.985)^(dt * 60)
    _wind_current = _wind_current + (_wind_target - _wind_current) * wind_lerp
    local vx, vy = camera.ViewportSize.X, camera.ViewportSize.Y
    _wind_time = _wind_time + dt
    for _, fl in ipairs(flakes) do
        local o = fl.obj
        if o and o.Parent then
            fl.sway        = fl.sway + dt * (1.2 + fl.layer * 0.3)
            fl.pulse_phase = fl.pulse_phase + dt * fl.pulse_speed
            local wind_factor = 0.5 + fl.layer * 0.3
            local px = o.Position.X.Offset + fl.drift + sin(fl.sway) * fl.sway_amp + _wind_current * wind_factor
            local py = o.Position.Y.Offset + fl.speed * (1 + abs(_wind_current) * 0.15)
            local pulse = (sin(fl.pulse_phase) * 0.12)
            local new_alpha = clamp(fl.base_alpha + pulse, 0.05, 1)
            o.BackgroundTransparency = 1 - new_alpha
            if py > vy + 10 then py = -10; px = random(0, vx) end
            if px < -10 then px = vx + 10 elseif px > vx + 10 then px = -10 end
            o.Position = dim2(0, px, 0, py)
        end
    end
end

conn(run_service.RenderStepped, update_snow)
...
function nebula:_apply_theme(name) nebula:set_theme(name) end
...
function nebula:notify(text, duration)
    ...
    local wrap = mk("Frame", {
        ...
        Position = dim2(0, -240, 0, 60 + (slot-1)*36),
        Size = dim2(0, 230, 0, 28), BackgroundTransparency = 0,
    })
    mk("UICorner", { Parent = wrap, CornerRadius = dim(0,5) })
    local wrap_stroke = mk("UIStroke", { Parent = wrap, Color = theme.accent_dim, Thickness = 1 })
    register(wrap, "BackgroundColor3", "bg_mid")
    register(wrap_stroke, "Color", "accent_dim")
    local bar = mk("Frame", {
        Parent = wrap, BackgroundColor3 = theme.accent, BorderSizePixel = 0,
        Size = dim2(0,3,1,0), BackgroundTransparency = 0
    })
    mk("UICorner", { Parent = bar, CornerRadius = dim(0,2) })
    register(bar, "BackgroundColor3", "accent")
    local lbl = mk("TextLabel", {
        Parent = wrap, BackgroundTransparency = 1,
        Position = dim2(0,10,0,0), Size = dim2(1,-14,1,0),
        Font = Enum.Font.GothamMedium, Text = text,
        TextColor3 = theme.text_main, TextSize = 12,
    ...
    tw_spring(wrap, { Position = dim2(0,12,0,60+(slot-1)*36) }, 0.45)
    task.spawn(function()
        local steps = floor(duration / 0.016)
        for i = 1, steps do
            local remaining = 1 - (i / steps)
            pcall(function() bar.Size = dim2(0, 3, remaining, 0) end)
            task.wait(0.016)
        end
    end)
    task.delay(duration, function()
        nebula.notifications[slot] = nil
        tw(wrap, { Position = dim2(0,-240,0,wrap.Position.Y.Offset) }, 0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In)
        task.wait(0.35)
        wrap:Destroy()
    end)
end
...
function nebula:watermark(opts)
    ...
    local frame = mk("Frame", {
        Parent = overlay_gui, BackgroundColor3 = theme.bg_mid, BorderSizePixel = 0,
        Position = dim2(0,12,0,50), Size = dim2(0,400,0,24),
    ...
    local lbl = mk("TextLabel", {
        ...
        TextSize = 12, TextColor3 = theme.text_accent,
    ...
    local function refresh()
        local parts = {}
        if cfg.brand   ~= "" then insert(parts, cfg.brand)   end
        if cfg.version ~= "" then insert(parts, cfg.version) end
        if cfg.date    ~= "" then insert(parts, cfg.date)    end
    ...
    refresh()
    task.spawn(function()
        while lbl.Parent do refresh(); task.wait(1) end
    end)
...
function nebula:keybind_list(opts)
    ...
    local C = {
        WIDTH=220, HEADER_H=24, LINE_H=1, PAD_TOP=6,
        PAD_BOT=8, PAD_L=8, PAD_R=8, ROW_H=16, ROW_GAP=4,
        MARGIN=12, OFFSET_Y=200,
    }
    local frame = mk("Frame", {
        Parent = overlay_gui, BackgroundColor3 = theme.bg_mid, BorderSizePixel = 0,
        Position = dim2(0,C.MARGIN,0,C.OFFSET_Y),
        Size = dim2(0,C.WIDTH,0,C.HEADER_H+C.LINE_H+C.PAD_TOP+C.ROW_H+C.PAD_BOT),
        ClipsDescendants = false, ZIndex = 1000, Visible = opts.visible ~= false,
    })
    mk("UICorner", { Parent = frame, CornerRadius = dim(0,4) })
    ...
    local header = mk("Frame", {
        Parent = frame, BackgroundColor3 = theme.bg_light, BorderSizePixel = 0,
        Position = dim2(0,0,0,0), Size = dim2(1,0,0,C.HEADER_H), ZIndex = 1001,
    })
    ...
    local header_lbl = mk("TextLabel", {
        ...
        Position = dim2(0,10,0,0), Size = dim2(1,-20,1,0),
        Font = Enum.Font.GothamBold, Text = "Keybinds",
        TextSize = 12, TextColor3 = theme.text_accent,
        TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 1002,
    })
    register(header_lbl, "TextColor3", "text_accent")
    local accent_line = mk("Frame", {
        Parent = frame, BackgroundColor3 = theme.accent, BorderSizePixel = 0,
        Position = dim2(0,0,0,C.HEADER_H), Size = dim2(1,0,0,C.LINE_H), ZIndex = 1001,
    })
    ...
    local body = mk("Frame", {
        ...
        Size = dim2(1,0,0,C.PAD_TOP+C.ROW_H+C.PAD_BOT), ZIndex = 1001,
    })
    local empty_lbl = mk("TextLabel", {
        Parent = body, BackgroundTransparency = 1,
        Position = dim2(0,C.PAD_L,0,C.PAD_TOP), Size = dim2(1,-C.PAD_L-C.PAD_R,0,C.ROW_H),
        Font = Enum.Font.Gotham, Text = "no keybinds",
        TextSize = 11, TextColor3 = theme.text_dim,
        TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 1002,
    })
...
function nebula:_rebuild_keybind_list()
    ...
    for _, entry in ipairs(nebula._keybind_entries) do
        ...
        local row = mk("Frame", {
            Parent = kl.body, BackgroundTransparency = 1, BorderSizePixel = 0,
            Position = dim2(0,C.PAD_L,0,y), Size = dim2(1,-C.PAD_L-C.PAD_R,0,C.ROW_H), ZIndex = 1002,
        })
        local name_lbl = mk("TextLabel", {
            Parent = row, BackgroundTransparency = 1,
            Position = dim2(0,0,0,0), Size = dim2(0.6,0,1,0),
            Font = Enum.Font.Gotham, Text = entry.name or "keybind",
            TextSize = 11, TextColor3 = theme.text_main,
            TextXAlignment = Enum.TextXAlignment.Left,
            TextTruncate = Enum.TextTruncate.AtEnd, ZIndex = 1003,
        })
        ...
        local key_lbl = mk("TextLabel", {
            Parent = row, BackgroundTransparency = 1,
            Position = dim2(0.6,0,0,0), Size = dim2(0.4,0,1,0),
            Font = Enum.Font.GothamBold, Text = key_text,
            TextSize = 11, TextColor3 = theme.text_accent,
            TextXAlignment = Enum.TextXAlignment.Right, ZIndex = 1003,
        })
    ...
    local body_h
    if shown == 0 then body_h = C.PAD_TOP + C.ROW_H + C.PAD_BOT
    else body_h = C.PAD_TOP + shown * C.ROW_H + max(0, shown-1) * C.ROW_GAP + C.PAD_BOT end
...
function nebula:window(opts)
    ...
    parent_gui(root_gui)
    local vx, vy = camera.ViewportSize.X, camera.ViewportSize.Y
    local target_x = vx/2 - 310
    local target_y = vy/2 - 270
    local shell = mk("Frame", {
        Parent = root_gui, BackgroundColor3 = theme.bg_dark, BorderSizePixel = 0,
        Position = dim2(0, target_x - 340, 0, target_y + 30),
        Size = dim2(0,620,0,540), ClipsDescendants = false,
    })
    ...
    make_shell_shade(shell)
    insert(nebula._shells, shell)
    local titlebar = mk("Frame", {
        Parent = shell, BackgroundColor3 = theme.bg_dark, BorderSizePixel = 0,
        Size = dim2(1,0,0,28),
    })
    mk("UICorner", { Parent = titlebar, CornerRadius = dim(0,6) })
    mk("Frame", {
        Parent = titlebar, BackgroundColor3 = theme.bg_dark, BorderSizePixel = 0,
    ...
    local title_lbl = mk("TextLabel", {
        ...
        Position = dim2(0,10,0,0), Size = dim2(0,0,1,0),
        AutomaticSize = Enum.AutomaticSize.X,
        Font = Enum.Font.GothamBold, Text = opts.name or "nebula",
        TextColor3 = theme.text_main, TextSize = 12,
        TextXAlignment = Enum.TextXAlignment.Left,
    })
    ...
    if opts.version then
        mk("TextLabel", {
            Parent = titlebar, BackgroundTransparency = 1,
            Position = dim2(1,-90,0,0), Size = dim2(0,80,1,0),
            Font = Enum.Font.Gotham, Text = opts.version,
            TextColor3 = theme.text_dim, TextSize = 11,
            TextXAlignment = Enum.TextXAlignment.Right,
        })
    end
    mk("Frame", {
        Parent = shell, BackgroundColor3 = theme.border, BorderSizePixel = 0,
        Position = dim2(0,0,0,28), Size = dim2(1,0,0,1),
    })
    local tabbar_clip = mk("Frame", {
        Parent = shell, BackgroundColor3 = theme.bg_dark, BorderSizePixel = 0,
        Position = dim2(0,0,0,29), Size = dim2(1,0,0,28), ClipsDescendants = true,
    })
    register(tabbar_clip, "BackgroundColor3", "bg_dark")
    local tabbar = mk("ScrollingFrame", {
        Parent = tabbar_clip, BackgroundTransparency = 1, BorderSizePixel = 0,
    ...
    mk("UIListLayout", {
        ...
        Padding = dim(0,1), SortOrder = Enum.SortOrder.LayoutOrder,
    })
    mk("UIPadding", {
        Parent = tabbar, PaddingLeft = dim(0,6),
    ...
    tabbar_clip.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseWheel then
            ...
            local max_x    = max(0, canvas_w - frame_w)
            local new_x    = clamp(tabbar.CanvasPosition.X - input.Position.Z * SCROLL_STEP, 0, max_x)
            tween_svc:Create(tabbar,
    ...
    mk("Frame", {
        Parent = shell, BackgroundColor3 = theme.border, BorderSizePixel = 0,
        Position = dim2(0,0,0,57), Size = dim2(1,0,0,1),
    })
    local content = mk("Frame", {
        Parent = shell, BackgroundTransparency = 1, BorderSizePixel = 0,
        Position = dim2(0,0,0,58), Size = dim2(1,0,1,-58), ClipsDescendants = true,
    })
    ...
    win._tabs       = {}
    if not nebula._blur then nebula._blur = ensure_blur() end
    local blur_effect = nebula._blur
    if not nebula._snow then
        nebula._snow, nebula._snow_flakes = make_snow(overlay_gui)
        nebula._snow.Visible = false
        nebula._vignette = make_vignette(overlay_gui)
        nebula._vignette.Visible = false
    end
    local snow_holder = nebula._snow
    local vignette    = nebula._vignette
    if not nebula._romania then
        nebula._romania = make_romania_tag(overlay_gui)
    end
    local romania = nebula._romania
    romania.holder.Visible = false
    local function do_open_anim()
        shell.Position = dim2(0, target_x - 340, 0, target_y + 30)
        shell.BackgroundTransparency = 0.85
        tw_spring(shell, {
            Position = dim2(0, target_x, 0, target_y),
            BackgroundTransparency = 0,
        }, 0.55)
        set_blur_amount(45, 0.5)
        tw(vignette, { BackgroundTransparency = 0.88 }, 0.45)
    end
    local function do_close_anim(on_done)
        tw(shell, {
            Position = dim2(0, target_x + 320, 0, target_y + 20),
            BackgroundTransparency = 0.85,
        }, 0.28, Enum.EasingStyle.Back, Enum.EasingDirection.In)
        set_blur_amount(0, 0.28)
        tw(vignette, { BackgroundTransparency = 1 }, 0.25)
        task.delay(0.32, function()
            if on_done then on_done() end
        end)
    end
    function win:set_visible(bool)
        root_gui.Enabled = bool
        if snow_holder then snow_holder.Visible = bool end
        if vignette    then vignette.Visible    = bool end
        if romania     then romania.holder.Visible = bool end
        if bool then
            do_open_anim()
        else
            root_gui.Enabled = true
            do_close_anim(function()
                root_gui.Enabled = false
                snow_holder.Visible = false
    ...
    conn(uis.InputBegan, function(input, gp)
        ...
        if input.KeyCode == win._toggle_key then
            win:set_visible(not root_gui.Enabled)
        end
    end)
    do_open_anim()
    snow_holder.Visible = true
    vignette.Visible    = true
    romania.holder.Visible = true
...
function nebula:tab(opts)
    ...
    local btn = mk("TextButton", {
        Parent = self._tabbar, AutoButtonColor = false,
        BackgroundColor3 = theme.bg_dark, BorderSizePixel = 0,
        Font = Enum.Font.GothamMedium, Size = dim2(0,0,1,0),
        AutomaticSize = Enum.AutomaticSize.X,
        Text = opts.name or "Tab", TextColor3 = theme.text_dim, TextSize = 12,
    })
    ...
    local indicator = mk("Frame", {
        Parent = btn, BackgroundColor3 = theme.accent, BorderSizePixel = 0,
        Position = dim2(0,0,1,-2), Size = dim2(0,0,0,2), Visible = false,
    })
    mk("UICorner", { Parent = indicator, CornerRadius = dim(0,2) })
    register(indicator, "BackgroundColor3", "accent")
    local container = mk("Frame", {
        Parent = self._content, BackgroundTransparency = 1,
        BorderSizePixel = 0, Size = dim2(1,0,1,0), Visible = false,
    })
    mk("UIPadding", {
        Parent = container,
    ...
    mk("UIListLayout", {
        ...
        Padding = dim(0,8), SortOrder = Enum.SortOrder.LayoutOrder,
    })
    tab._btn       = btn
    ...
    local function scroll_to_btn()
        ...
        local tgt_x   = cur_pos
        if btn_x < cur_pos then tgt_x = btn_x - 6
        elseif btn_x + btn_w > cur_pos + frame_w then tgt_x = btn_x + btn_w - frame_w + 6 end
        tgt_x = clamp(tgt_x, 0, max_x)
        tween_svc:Create(tabbar,
            TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            { CanvasPosition = Vector2.new(tgt_x,0) }
        ):Play()
    end
    function tab:select()
        if nebula.current_tab and nebula.current_tab ~= tab then
            local prev = nebula.current_tab
            prev._btn.TextColor3       = theme.text_dim
            prev._btn.BackgroundColor3 = theme.bg_dark
        ...
        indicator.Size    = dim2(1,0,0,2)
        container.Visible  = true
        container.Position = dim2(0,0,0,0)
...
function nebula:column()
    local col = {}
    local frame = mk("Frame", {
        Parent = self._container, BackgroundTransparency = 1,
    ...
    mk("UIListLayout", {
        Parent = scroll, Padding = dim(0,6), SortOrder = Enum.SortOrder.LayoutOrder,
    })
    col.holder = scroll
...
function nebula:section(opts)
    ...
    local outer = mk("Frame", {
        Parent = self.holder, BackgroundColor3 = theme.bg_mid, BorderSizePixel = 0,
    ...
    local header = mk("Frame", {
        Parent = outer, BackgroundColor3 = theme.bg_light, BorderSizePixel = 0,
        Size = dim2(1,0,0,26),
    })
    mk("UICorner", { Parent = header, CornerRadius = dim(0,4) })
    mk("Frame", {
    ...
    local sec_title = mk("TextLabel", {
        Parent = header, BackgroundTransparency = 1,
        Position = dim2(0,10,0,0), Size = dim2(1,-10,1,0),
        Font = Enum.Font.GothamBold, Text = opts.name or "section",
        TextColor3 = theme.text_accent, TextSize = 11,
        TextXAlignment = Enum.TextXAlignment.Left,
    })
    register(sec_title, "TextColor3", "text_accent")
    local accent_line = mk("Frame", {
        Parent = outer, BackgroundColor3 = theme.accent, BorderSizePixel = 0,
        Position = dim2(0,0,0,26), Size = dim2(1,0,0,1),
    })
    register(accent_line, "BackgroundColor3", "accent")
    local body = mk("Frame", {
        Parent = outer, BackgroundTransparency = 1, BorderSizePixel = 0,
        Position = dim2(0,0,0,27), Size = dim2(1,0,0,0),
        AutomaticSize = Enum.AutomaticSize.Y,
    })
    mk("UIPadding", {
        Parent = body,
        PaddingTop = dim(0,6), PaddingBottom = dim(0,8),
        PaddingLeft = dim(0,8), PaddingRight = dim(0,8),
    })
    mk("UIListLayout", {
        Parent = body, Padding = dim(0,5), SortOrder = Enum.SortOrder.LayoutOrder,
    })
    sec.holder = body
    return setmetatable(sec, nebula)
end

local TOGGLE_TRACK_W   = 28
local TOGGLE_TRACK_H   = 14
local TOGGLE_KNOB_SIZE = 10
local TOGGLE_KNOB_PAD  = 2
...
function nebula:toggle(opts)
    ...
    local row = mk("Frame", {
        Parent = self.holder, BackgroundTransparency = 1,
        BorderSizePixel = 0, Size = dim2(1,0,0,20),
    })
    local track = mk("Frame", {
        Parent = row, BackgroundColor3 = theme.bg_element, BorderSizePixel = 0,
        Position = dim2(0,0,0.5,-TOGGLE_TRACK_H/2),
        Size = dim2(0,TOGGLE_TRACK_W,0,TOGGLE_TRACK_H),
    })
    mk("UICorner", { Parent = track, CornerRadius = dim(0, TOGGLE_TRACK_H/2) })
    local track_stroke = mk("UIStroke", { Parent = track, Color = theme.border, Thickness = 1 })
    register(track_stroke, "Color", "border")
    local knob = mk("Frame", {
        Parent = track, BackgroundColor3 = theme.text_dim, BorderSizePixel = 0,
        Position = dim2(0,TOGGLE_KNOB_PAD,0.5,-TOGGLE_KNOB_SIZE/2),
        Size = dim2(0,TOGGLE_KNOB_SIZE,0,TOGGLE_KNOB_SIZE),
    })
    mk("UICorner", { Parent = knob, CornerRadius = dim(0, TOGGLE_KNOB_SIZE/2) })
    local lbl = mk("TextLabel", {
        Parent = row, BackgroundTransparency = 1,
        Position = dim2(0, TOGGLE_TRACK_W+8, 0, 0),
        Size = dim2(1, -(TOGGLE_TRACK_W+8), 1, 0),
        Font = Enum.Font.Gotham, Text = cfg.name,
        TextColor3 = theme.text_main, TextSize = 12,
        TextXAlignment = Enum.TextXAlignment.Left,
    })
    local btn = mk("TextButton", {
        Parent = row, BackgroundTransparency = 1, BorderSizePixel = 0,
        Size = dim2(1,0,1,0), Text = "",
    })
    local knob_off_x = TOGGLE_KNOB_PAD
    local knob_on_x  = TOGGLE_TRACK_W - TOGGLE_KNOB_SIZE - TOGGLE_KNOB_PAD
    local function animate_toggle(state)
        if state then
            tween_svc:Create(track, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                { BackgroundColor3 = theme.accent }):Play()
            tween_svc:Create(knob, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                { Position = dim2(0, knob_on_x, 0.5, -TOGGLE_KNOB_SIZE/2),
                  BackgroundColor3 = Color3.new(1,1,1),
                  Size = dim2(0, TOGGLE_KNOB_SIZE, 0, TOGGLE_KNOB_SIZE) }):Play()
            tw(lbl, { TextColor3 = theme.text_accent }, 0.18)
        else
            tween_svc:Create(track, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { BackgroundColor3 = theme.bg_element }):Play()
            tween_svc:Create(knob, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { Position = dim2(0, knob_off_x, 0.5, -TOGGLE_KNOB_SIZE/2),
                  BackgroundColor3 = theme.text_dim,
                  Size = dim2(0, TOGGLE_KNOB_SIZE, 0, TOGGLE_KNOB_SIZE) }):Play()
            tw(lbl, { TextColor3 = theme.text_main }, 0.18)
        end
    end
    btn.MouseButton1Down:Connect(function()
        tween_svc:Create(knob, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            { Size = dim2(0, TOGGLE_KNOB_SIZE + 2, 0, TOGGLE_KNOB_SIZE - 1) }):Play()
    end)
    ...
    insert(theme_change_callbacks, function()
        if cfg.enabled then
            ...
            lbl.TextColor3         = theme.text_accent
        else
            track.BackgroundColor3 = theme.bg_element
    ...
    if cfg.enabled then
        track.BackgroundColor3 = theme.accent
        knob.Position = dim2(0, knob_on_x, 0.5, -TOGGLE_KNOB_SIZE/2)
        knob.BackgroundColor3 = Color3.new(1,1,1)
        lbl.TextColor3 = theme.text_accent
    end
...
function nebula:toggle_color(opts)
    opts = opts or {}
    local t_cfg = {
        flag          = opts.flag       or tostring(random(1,99999)),
        color_flag    = opts.color_flag or tostring(random(1,99999)),
        name          = opts.name       or "Toggle",
        enabled       = opts.default    or false,
        color         = opts.color      or Color3.new(1,1,1),
        alpha         = opts.alpha      or 1,
        callback      = opts.callback   or function() end,
        color_callback= opts.color_callback or function() end,
        visible       = opts.visible ~= false,
        open          = false,
    }
    ...
    local SWATCH_W    = 16
    local SWATCH_PAD  = 6
    local lbl_offset  = TOGGLE_TRACK_W + 8

    local lbl = mk("TextLabel", {
        Parent = row, BackgroundTransparency = 1,
        Position = dim2(0, lbl_offset, 0, 0),
        Size = dim2(1, -(lbl_offset + SWATCH_W + SWATCH_PAD), 1, 0),
        Font = Enum.Font.Gotham, Text = t_cfg.name,
        TextColor3 = theme.text_main, TextSize = 12,
        TextXAlignment = Enum.TextXAlignment.Left,
    })

    local swatch = mk("TextButton", {
        Parent = row, BackgroundColor3 = t_cfg.color, BorderSizePixel = 0,
        Position = dim2(1, -(SWATCH_W), 0.5, -8),
        Size = dim2(0, SWATCH_W, 0, 16),
    ...
    local toggle_btn = mk("TextButton", {
        Parent = row, BackgroundTransparency = 1, BorderSizePixel = 0,
        Position = dim2(0,0,0,0),
        Size = dim2(1, -(SWATCH_W + SWATCH_PAD), 1, 0),
        Text = "", ZIndex = 3,
    })

    local h_cp, s_cp, v_cp = t_cfg.color:ToHSV()
    local a_cp = t_cfg.alpha
    local dsv, dhue, dal = false, false, false

    local picker = mk("Frame", {
        Parent = popup_root, BackgroundColor3 = theme.bg_mid, BorderSizePixel = 0,
        Size = dim2(0,180,0,168), Visible = false, ZIndex = 300,
    })
    ...
    local sv_frame = mk("Frame", {
        Parent = picker, BorderSizePixel = 0,
        Size = dim2(1,-20,1,-32), BackgroundColor3 = Color3.fromHSV(h_cp,1,1), ZIndex = 301,
    })
    mk("UICorner", { Parent = sv_frame, CornerRadius = dim(0,3) })

    local sat_layer = mk("Frame", {
        Parent = sv_frame, BorderSizePixel = 0, Size = dim2(1,0,1,0),
        BackgroundColor3 = Color3.new(1,1,1), ZIndex = 302,
    })
    mk("UIGradient", {
        Parent = sat_layer,
        Color = ColorSequence.new{
    ...
    local val_layer = mk("Frame", {
        Parent = sv_frame, BorderSizePixel = 0, Size = dim2(1,0,1,0),
        BackgroundColor3 = Color3.new(0,0,0), ZIndex = 303,
    })
    mk("UIGradient", {
        Parent = val_layer, Rotation = 270,
        Color = ColorSequence.new{
            ColorSequenceKeypoint.new(0, Color3.new(0,0,0)),
    ...
    local sv_cursor = mk("Frame", {
        Parent = sv_frame, BackgroundColor3 = Color3.new(1,1,1),
        BorderSizePixel = 1, Size = dim2(0,6,0,6), ZIndex = 304,
    })
    mk("UICorner", { Parent = sv_cursor, CornerRadius = dim(0,3) })
    local sv_btn = mk("TextButton", {
        Parent = sv_frame, BackgroundTransparency = 1,
    ...
    local hue_bar = mk("TextButton", {
        ...
        Position = dim2(1,-14,0,0), Size = dim2(0,10,1,-32),
    ...
    local hue_cursor = mk("Frame", {
        Parent = hue_bar, BackgroundColor3 = Color3.new(1,1,1),
        BorderSizePixel = 1, Size = dim2(1,0,0,3), ZIndex = 302,
    })

    local alpha_bar = mk("TextButton", {
        Parent = picker, BackgroundColor3 = rgb(255,255,255), BorderSizePixel = 0,
        Position = dim2(0,0,1,-20), Size = dim2(1,-20,0,10),
        Text = "", AutoButtonColor = false, ZIndex = 301,
    })
    mk("UICorner", { Parent = alpha_bar, CornerRadius = dim(0,3) })
    local alpha_fill = mk("Frame", {
        Parent = alpha_bar, BorderSizePixel = 0, Size = dim2(1,0,1,0), ZIndex = 302,
    })
    ...
    local alpha_cursor = mk("Frame", {
        Parent = alpha_bar, BackgroundColor3 = Color3.new(1,1,1),
        BorderSizePixel = 1, Size = dim2(0,3,1,0), ZIndex = 303,
    })

    local closing_conn_cp = nil

    local function reposition_tc()
        if not picker.Parent then return end
        local abs = swatch.AbsolutePosition
        picker.Position = dim2(0, abs.X - 188, 0, abs.Y + gui_offset)
    end
    insert(reposition_callbacks, reposition_tc)

    local function set_color(col, al)
        if col then h_cp, s_cp, v_cp = col:ToHSV() end
        if al  then a_cp = al end
        local c = Color3.fromHSV(h_cp, s_cp, v_cp)
        sv_frame.BackgroundColor3   = Color3.fromHSV(h_cp,1,1)
        alpha_fill.BackgroundColor3 = c
        sv_cursor.Position          = dim2(s_cp,-3,1-v_cp,-3)
        hue_cursor.Position         = dim2(0,0,1-h_cp,-1)
        alpha_cursor.Position       = dim2(a_cp,-1,0,0)
        tween_svc:Create(swatch, TweenInfo.new(0.1), { BackgroundColor3 = c }):Play()
        t_cfg.color = c; t_cfg.alpha = a_cp
        flags[t_cfg.color_flag] = { Color=c, Transparency=a_cp }
        t_cfg.color_callback(c, a_cp)
    end

    local function set_picker_visible(bool)
        if bool then
            if nebula.current_element_open and nebula.current_element_open ~= t_cfg then
                nebula.current_element_open.set_visible(false)
                nebula.current_element_open.open = false
            end
            nebula.current_element_open = t_cfg
            t_cfg.open  = true
            picker.Visible = true
            reposition_tc()
            closing_conn_cp = uis.InputBegan:Connect(function(input, gp)
                if gp then return end
                if input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
                local m      = uis:GetMouseLocation()
                local p_abs  = picker.AbsolutePosition
                local p_sz   = picker.AbsoluteSize
                local s_abs  = swatch.AbsolutePosition
                local s_sz   = swatch.AbsoluteSize
                local in_p   = m.X>=p_abs.X and m.X<=p_abs.X+p_sz.X and m.Y>=p_abs.Y and m.Y<=p_abs.Y+p_sz.Y
                local in_s   = m.X>=s_abs.X and m.X<=s_abs.X+s_sz.X and m.Y>=s_abs.Y and m.Y<=s_abs.Y+s_sz.Y
                if in_p or in_s then return end
                if is_inside_any_shell(m.X, m.Y) then return end
                set_picker_visible(false)
            end)
        else
            if closing_conn_cp then closing_conn_cp:Disconnect(); closing_conn_cp = nil end
            picker.Visible = false
            t_cfg.open = false
        end
    end

    t_cfg.set_visible = set_picker_visible

    local function upd_color()
        local m = uis:GetMouseLocation()
        if dsv then
            s_cp = clamp((m.X - sv_frame.AbsolutePosition.X)/sv_frame.AbsoluteSize.X, 0, 1)
            v_cp = 1 - clamp((m.Y - gui_offset - sv_frame.AbsolutePosition.Y)/sv_frame.AbsoluteSize.Y, 0, 1)
        elseif dhue then
            h_cp = 1 - clamp((m.Y - gui_offset - hue_bar.AbsolutePosition.Y)/hue_bar.AbsoluteSize.Y, 0, 1)
        elseif dal then
            a_cp = clamp((m.X - alpha_bar.AbsolutePosition.X)/alpha_bar.AbsoluteSize.X, 0, 1)
        end
        set_color()
    end
    ...
    uis.InputChanged:Connect(function(i)
        if (dsv or dhue or dal) and i.UserInputType == Enum.UserInputType.MouseMovement then upd_color() end
    end)

    swatch.MouseButton1Click:Connect(function() set_picker_visible(not t_cfg.open) end)
    ...
    function t_cfg.set(bool)
        t_cfg.enabled = bool
        animate_toggle(bool)
        flags[t_cfg.flag] = bool
        t_cfg.callback(bool)
    end

    function t_cfg.set_color(col, al) set_color(col, al) end
    function t_cfg.set_element_visible(b) row.Visible = b end

    insert(theme_change_callbacks, function()
        if t_cfg.enabled then
            track.BackgroundColor3 = theme.accent
            knob.BackgroundColor3  = Color3.new(1,1,1)
            lbl.TextColor3         = theme.text_accent
        else
    ...
    toggle_btn.MouseButton1Click:Connect(function() t_cfg.set(not t_cfg.enabled) end)
    ...
    set_color(t_cfg.color, t_cfg.alpha)

    t_cfg.set_element_visible(t_cfg.visible)
    config_flags[t_cfg.flag]       = t_cfg.set
    config_flags[t_cfg.color_flag] = t_cfg.set_color
    nebula.visible_flags[t_cfg.flag] = t_cfg.set_element_visible
    return setmetatable(t_cfg, nebula)
end
...
function nebula:slider(opts)
    ...
    if cfg.name then
        mk("TextLabel", {
            Parent = wrap, BackgroundTransparency = 1,
            Position = dim2(0,0,0,0), Size = dim2(1,0,0,14),
            Font = Enum.Font.Gotham, Text = cfg.name,
            TextColor3 = theme.text_dim, TextSize = 11,
            TextXAlignment = Enum.TextXAlignment.Left,
        })
    ...
    local track = mk("Frame", {
        Parent = wrap, BackgroundColor3 = theme.bg_element, BorderSizePixel = 0,
        Position = dim2(0,0,0,ty), Size = dim2(1,0,0,12),
    })
    mk("UICorner", { Parent = track, CornerRadius = dim(0,3) })
    local track_stroke = mk("UIStroke", { Parent = track, Color = theme.border, Thickness = 1 })
    register(track_stroke, "Color", "border")
    local fill = mk("Frame", {
        Parent = track, BackgroundColor3 = theme.accent,
        BorderSizePixel = 0, Size = dim2(0,0,1,0),
    })
    mk("UICorner", { Parent = fill, CornerRadius = dim(0,3) })
    register(fill, "BackgroundColor3", "accent")
    local val_lbl = mk("TextLabel", {
        Parent = track, BackgroundTransparency = 1,
        Size = dim2(1,0,1,0), Font = Enum.Font.Gotham,
        Text = "", TextColor3 = theme.text_main, TextSize = 10,
    })
    register(val_lbl, "TextColor3", "text_main")
    local drag_btn = mk("TextButton", {
        Parent = track, BackgroundTransparency = 1, BorderSizePixel = 0,
        Size = dim2(1,0,1,0), Text = "", ZIndex = 5,
    })
    local function ri(n)
    ...
    function cfg.set(val)
        ...
        tween_svc:Create(fill, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            { Size = dim2(pct,0,1,0) }):Play()
...
function nebula:dropdown(opts)
    ...
    local dd_btn = mk("TextButton", {
        Parent = wrap, BackgroundColor3 = theme.bg_element,
        BackgroundTransparency = 1, BorderSizePixel = 0,
        Position = dim2(0,0,0,by), Size = dim2(1,0,0,18),
        Text = "", AutoButtonColor = false,
    })
    mk("UICorner", { Parent = dd_btn, CornerRadius = dim(0,3) })
    local lbl = mk("TextLabel", {
        Parent = dd_btn, BackgroundTransparency = 1,
        Position = dim2(0,4,0,0), Size = dim2(1,-22,1,0),
        Font = Enum.Font.Gotham, Text = "select",
        TextColor3 = theme.text_main, TextSize = 11,
        TextXAlignment = Enum.TextXAlignment.Left,
        TextTruncate = Enum.TextTruncate.AtEnd,
    })
    register(lbl, "TextColor3", "text_main")
    local arrow_lbl = mk("TextLabel", {
        Parent = dd_btn, BackgroundTransparency = 1,
        Position = dim2(1,-14,0,0), Size = dim2(0,12,1,0),
        Font = Enum.Font.GothamBold, Text = "+",
        TextColor3 = theme.text_dim, TextSize = 14,
    })
    local popup = mk("Frame", {
        Parent = popup_root, BackgroundColor3 = theme.bg_mid, BorderSizePixel = 0,
        Size = dim2(0,100,0,0), Visible = false, ZIndex = 200, ClipsDescendants = true,
    })
    ...
    mk("UIListLayout", {
        Parent = popup_inner, Padding = dim(0,2), SortOrder = Enum.SortOrder.LayoutOrder,
    })
    local closing_conn   = nil
    local function calc_popup_h()
        return clamp(#cfg.items * 18 + 8, 0, 160)
    end
    local function reposition_dd()
        ...
        popup.Size     = dim2(0, sz.X, 0, popup.Size.Y.Offset)
    end
    ...
    function cfg.set_visible(bool)
        if bool then
            ...
            local popup_target_h = calc_popup_h()
            popup.Size     = dim2(0, dd_btn.AbsoluteSize.X, 0, 0)
            popup.Visible  = true
            cfg.open       = true
            reposition_dd()
            tween_svc:Create(popup, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                { Size = dim2(0, dd_btn.AbsoluteSize.X, 0, popup_target_h) }):Play()
            closing_conn = uis.InputBegan:Connect(function(input, gp)
                if gp then return end
                if input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
                local m        = uis:GetMouseLocation()
                local pop_abs  = popup.AbsolutePosition
                local pop_size = popup.AbsoluteSize
                local btn_abs  = dd_btn.AbsolutePosition
                local btn_size = dd_btn.AbsoluteSize
                local in_pop   = m.X>=pop_abs.X and m.X<=pop_abs.X+pop_size.X and m.Y>=pop_abs.Y and m.Y<=pop_abs.Y+pop_size.Y
                local in_btn   = m.X>=btn_abs.X and m.X<=btn_abs.X+btn_size.X and m.Y>=btn_abs.Y and m.Y<=btn_abs.Y+btn_size.Y
                if in_pop or in_btn then return end
                if is_inside_any_shell(m.X, m.Y) then return end
                cfg.set_visible(false)
            end)
        else
            if closing_conn then closing_conn:Disconnect(); closing_conn = nil end
            tween_svc:Create(popup, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
                { Size = dim2(0, popup.Size.X.Offset, 0, 0) }):Play()
    ...
    function cfg:refresh_options(list)
        ...
        for _, item in ipairs(list) do
            local opt = mk("TextButton", {
                Parent = popup_inner, BackgroundTransparency = 1, BorderSizePixel = 0,
                Font = Enum.Font.Gotham, Size = dim2(1,0,0,16),
                Text = item, TextColor3 = theme.text_main,
            ...
            opt.MouseEnter:Connect(function()
                tween_svc:Create(opt, TweenInfo.new(0.1), { TextColor3 = theme.text_accent }):Play()
            end)
            opt.MouseLeave:Connect(function()
                local c = find(cfg.selected, item) and theme.text_accent or theme.text_main
                tween_svc:Create(opt, TweenInfo.new(0.1), { TextColor3 = c }):Play()
            end)
        end
    end
...
function nebula:textbox(opts)
    ...
    local wrap = mk("Frame", {
        Parent = self.holder, BackgroundColor3 = theme.bg_element,
    ...
    local tb = mk("TextBox", {
        Parent = wrap, BackgroundTransparency = 1, BorderSizePixel = 0,
        ClearTextOnFocus = false, Font = Enum.Font.Gotham,
        PlaceholderColor3 = theme.text_dim, PlaceholderText = cfg.placeholder,
        Position = dim2(0,8,0,0), Size = dim2(1,-16,1,0),
        Text = "", TextColor3 = theme.text_main,
        TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left,
    })
    register(tb, "TextColor3", "text_main")
    register(tb, "PlaceholderColor3", "text_dim")
    tb.Focused:Connect(function() tw(wrap_stroke, { Color = theme.accent, Thickness = 1.5 }, 0.15) end)
    tb.FocusLost:Connect(function() tw(wrap_stroke, { Color = theme.border, Thickness = 1 }, 0.15) end)
...
function nebula:button(opts)
    opts = opts or {}
    local cfg = {
        name     = opts.name or opts.text or "Button",
        callback = opts.callback or function() end,
        flag     = opts.flag or tostring(random(1,99999)),
        visible  = opts.visible ~= false,
    }
    local btn = mk("TextButton", {
        Parent = self.holder, BackgroundColor3 = theme.bg_element, BorderSizePixel = 0,
        Font = Enum.Font.GothamMedium, Size = dim2(1,0,0,18),
    ...
    btn.MouseEnter:Connect(function()
        tw(btn, { BackgroundColor3=theme.bg_light, TextColor3=theme.text_accent }, 0.12)
        tw(btn_stroke, { Color = theme.accent_dim, Thickness = 1 }, 0.12)
    end)
    btn.MouseLeave:Connect(function()
        tw(btn, { BackgroundColor3=theme.bg_element, TextColor3=theme.text_main }, 0.12)
        tw(btn_stroke, { Color = theme.border, Thickness = 1 }, 0.12)
    end)
    btn.MouseButton1Down:Connect(function()
        tw(btn, { BackgroundColor3=theme.accent_dim, Size=dim2(1,-2,0,16) }, 0.08)
    end)
    btn.MouseButton1Up:Connect(function()
        tw(btn, { BackgroundColor3=theme.bg_light, Size=dim2(1,0,0,18) }, 0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
        cfg.callback()
    end)
...
function nebula:label(opts)
    ...
    local lbl = mk("TextLabel", {
        Parent = row, BackgroundTransparency = 1, Size = dim2(1,0,1,0),
    ...
    function cfg.change_text(t)
        cfg.text = tostring(t); lbl.Text = cfg.prefix..cfg.text..cfg.suffix
    end
    ...
    function cfg.set_update(fn,iv) cfg.update = fn; if iv then cfg.interval = iv end end
    function cfg.set_element_visible(b) row.Visible = b end
    cfg.set_element_visible(cfg.visible)
...
function nebula:keybind(opts)
    opts = opts or {}
    local cfg = {
        flag          = opts.flag or "KB_"..tostring(random(1,99999)),
        name          = opts.name or "Keybind",
        key           = opts.key or nil,
        mode          = opts.mode or "toggle",
        active        = opts.default or false,
        callback      = opts.callback or function() end,
        on_bind       = opts.on_bind or nil,
        visible       = opts.visible ~= false,
        binding       = nil,
        show_in_list  = opts.show_in_list ~= false,
        _kl_key_lbl   = nil,
    }
    ...
    local kb_label = mk("TextLabel", {
        Parent = row, BackgroundTransparency = 1,
        Position = dim2(0,0,0,0), Size = dim2(1,-50,1,0),
        Font = Enum.Font.Gotham, Text = cfg.name,
        TextColor3 = theme.text_main, TextSize = 12,
        TextXAlignment = Enum.TextXAlignment.Left,
    })
    register(kb_label, "TextColor3", "text_main")
    local kb_btn = mk("TextButton", {
        Parent = row, BackgroundColor3 = theme.bg_element, BorderSizePixel = 0,
        Position = dim2(1,-44,0.5,-8), Size = dim2(0,44,0,16),
    ...
    function cfg.set(input)
        if type(input) == "table" then
            cfg.active = input.active; cfg.key = input.key; cfg.mode = input.mode
            local k = cfg.key
        ...
        elseif type(input) == "boolean" then
            cfg.active = input; cfg.callback(input)
        elseif typeof(input) == "EnumItem" then
            cfg.key = input; kb_btn.Text = string.lower(input.Name)
            tw(kb_btn, { TextColor3 = theme.text_accent }, 0.12)
            if cfg.on_bind then pcall(cfg.on_bind, cfg.key) end
            sync_kl_label()
        end
        flags[cfg.flag] = { active=cfg.active, key=cfg.key, mode=cfg.mode }
    ...
    kb_btn.MouseButton1Click:Connect(function()
        ...
        kb_btn.Text = "..."
        tw(kb_btn, { TextColor3 = theme.accent }, 0.1)
        tw(kb_stroke, { Color = theme.accent }, 0.1)
        cfg.binding = conn(uis.InputBegan, function(i, gp)
            ...
            if i.KeyCode == Enum.KeyCode.Escape then
                ...
                kb_btn.Text = (k and typeof(k)=="EnumItem" and k~=Enum.KeyCode.Unknown)
                    and string.lower(k.Name) or "none"
                tw(kb_btn,    { TextColor3 = theme.text_dim }, 0.1)
                tw(kb_stroke, { Color = theme.border },         0.1)
                cfg.binding:Disconnect(); cfg.binding = nil; return
            end
            cfg.set(i.KeyCode)
            tw(kb_stroke, { Color = theme.border }, 0.15)
...
function nebula:colorpicker(opts)
    ...
    local cfg = {
        ...
        alpha        = opts.alpha or 1,
        callback     = opts.callback or function() end,
        on_theme_key = opts.on_theme_key or nil,
        open         = false,
    }
    ...
    local cp_label = mk("TextLabel", {
        Parent = row, BackgroundTransparency = 1,
        Position = dim2(0,0,0,0), Size = dim2(1,-32,1,0),
        Font = Enum.Font.Gotham, Text = cfg.name,
        TextColor3 = theme.text_main, TextSize = 12,
        TextXAlignment = Enum.TextXAlignment.Left,
    })
    ...
    local swatch = mk("TextButton", {
        Parent = row, BackgroundColor3 = cfg.color, BorderSizePixel = 0,
        Position = dim2(1,-26,0.5,-8), Size = dim2(0,26,0,16),
        Text = "", AutoButtonColor = false,
    })
    mk("UICorner", { Parent = swatch, CornerRadius = dim(0,3) })
    local swatch_stroke = mk("UIStroke", { Parent = swatch, Color = theme.border, Thickness = 1 })
    ...
    local sat_layer = mk("Frame", {
        Parent = sv_frame, BorderSizePixel = 0, Size = dim2(1,0,1,0),
        BackgroundColor3 = Color3.new(1,1,1), BackgroundTransparency = 0, ZIndex = 302,
    })
    mk("UIGradient", {
        Parent = sat_layer,
    ...
    local val_layer = mk("Frame", {
        Parent = sv_frame, BorderSizePixel = 0, Size = dim2(1,0,1,0),
        BackgroundColor3 = Color3.new(0,0,0), BackgroundTransparency = 0, ZIndex = 303,
    })
    mk("UIGradient", {
        Parent = val_layer, Rotation = 270,
    ...
    function cfg.set_visible(bool)
        if bool then
            ...
            nebula.current_element_open = cfg
            picker.Visible = true; cfg.open = true; reposition_cp()
            closing_conn = uis.InputBegan:Connect(function(input, gp)
                if gp then return end
                if input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
                local m      = uis:GetMouseLocation()
                local p_abs  = picker.AbsolutePosition; local p_size = picker.AbsoluteSize
                local s_abs  = swatch.AbsolutePosition; local s_size = swatch.AbsoluteSize
                local in_p = m.X>=p_abs.X and m.X<=p_abs.X+p_size.X and m.Y>=p_abs.Y and m.Y<=p_abs.Y+p_size.Y
                local in_s = m.X>=s_abs.X and m.X<=s_abs.X+s_size.X and m.Y>=s_abs.Y and m.Y<=s_abs.Y+s_size.Y
                if in_p or in_s then return end
                if is_inside_any_shell(m.X, m.Y) then return end
                cfg.set_visible(false)
            end)
            ...
            if closing_conn then closing_conn:Disconnect(); closing_conn = nil end
            picker.Visible = false; cfg.open = false
        end
    end
    ...
    alpha_bar.MouseButton1Down:Connect(function()  dal  = true end)
    uis.InputEnded:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 then dsv, dhue, dal = false, false, false end
    end)
    uis.InputChanged:Connect(function(i)
...
function nebula:config_section(opts)
    ...
    local dd = sec:dropdown({ flag=DD_FLAG, items=list_cfgs(), callback=function() end })
    ...
    sec:button({ name="Save Config", callback=function()
        ...
        pcall(function() writefile(nebula.directory.."/configs/"..n..".cfg", nebula:get_config()) end)
        dd:refresh_options(list_cfgs()); nebula:notify("saved: "..n)
    end })
    sec:button({ name="Load Config", callback=function()
        ...
        pcall(function()
            ...
            if isfile(p) then nebula:load_config(readfile(p)); ok2 = true end
        end)
        if ok2 then nebula:notify("loaded: "..n) else nebula:notify("file not found", 2) end
    end })
    sec:button({ name="Delete Config", callback=function()
        ...
        dd:refresh_options(list_cfgs()); nebula:notify("deleted: "..n)
    end })
end
...
function nebula:theme_section()
    ...
    local dd = sec:dropdown({
        name = "Preset", flag = "NEBULA_THEME_PRESET",
...
function nebula:destroy()
    for _, c in ipairs(nebula.connections) do pcall(function() c:Disconnect() end) end
    nebula.connections = {}
    for _, sh in ipairs(nebula._shells) do
        pcall(function() if sh and sh.Parent then sh.Parent:Destroy() end end)
    end
    nebula._shells = {}
    ...
    pcall(function()
        local lit = game:GetService("Lighting")
        for _, v in ipairs(lit:GetChildren()) do
            if v.Name == "NebulaBlur" or v.Name == "NebulaCC" or v.Name == "NebulaDOF" then v:Destroy() end
        end
    end)
    nebula._blur = nil; nebula._color_correction = nil; nebula._dof = nil
    nebula._snow = nil; nebula._snow_flakes = nil; nebula._vignette = nil
    nebula._romania = nil; nebula._watermark = nil; nebula._keybind_list = nil
    nebula._keybind_entries = {}; nebula.flags = {}; nebula.config_flags = {}; nebula.visible_flags = {}
    if getgenv().Nebula == nebula then getgenv().Nebula = nil end
end
