...
local function set_blur_amount(target_size, duration)
    ...
    task.spawn(function()
        ...
        pcall(function()
            b.Size            = target_size
            cc.Saturation     = target_s
            dof.FarIntensity  = target_far
            dof.NearIntensity = target_near
        end)
    end)
end
...
local function make_romania_tag(parent)
    local holder = mk("Frame", {
        ...
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Position = dim2(1, -250, 1, -38),
        Size = dim2(0, 238, 0, 26),
        ZIndex = 1000,
        Visible = false,
    })
...
local function make_shell_shade(shell)
    ...
    local top_shadow = mk("Frame", {
        Parent = shade, BackgroundColor3 = Color3.new(0,0,0),
        BackgroundTransparency = 0, BorderSizePixel = 0,
        Size = dim2(1,0,0,80), Position = dim2(0,0,0,0), ZIndex = 6,
    })
    mk("UIGradient", {
        Parent = top_shadow, Rotation = 90,
        Transparency = NumberSequence.new{
    ...
    local bottom_shadow = mk("Frame", {
        Parent = shade, BackgroundColor3 = Color3.new(0,0,0),
        BackgroundTransparency = 0, BorderSizePixel = 0,
        Size = dim2(1,0,0,80), Position = dim2(0,0,1,-80), ZIndex = 6,
    })
    mk("UIGradient", {
        Parent = bottom_shadow, Rotation = 270,
        Transparency = NumberSequence.new{
            NumberSequenceKeypoint.new(0, 0.55),
            NumberSequenceKeypoint.new(1, 1),
        },
    ...
    local left_shadow = mk("Frame", {
        Parent = shade, BackgroundColor3 = Color3.new(0,0,0),
        BackgroundTransparency = 0, BorderSizePixel = 0,
        Size = dim2(0,60,1,0), Position = dim2(0,0,0,0), ZIndex = 6,
    })
    mk("UIGradient", {
        Parent = left_shadow, Rotation = 0,
        Transparency = NumberSequence.new{
    ...
    local right_shadow = mk("Frame", {
        Parent = shade, BackgroundColor3 = Color3.new(0,0,0),
        BackgroundTransparency = 0, BorderSizePixel = 0,
        Size = dim2(0,60,1,0), Position = dim2(1,-60,0,0), ZIndex = 6,
    })
    mk("UIGradient", {
        Parent = right_shadow, Rotation = 180,
        Transparency = NumberSequence.new{
            NumberSequenceKeypoint.new(0, 0.45),
            NumberSequenceKeypoint.new(1, 1),
        },
    ...
    for layer_idx, layer in ipairs(SNOW_LAYERS) do
        for _ = 1, layer.count do
            ...
            local f = mk("Frame", {
                ...
                ZIndex = 900 + layer_idx,
            })
            mk("UICorner", { Parent = f, CornerRadius = dim(1,0) })
            insert(flakes, {
                obj         = f,
                speed       = layer.speed_min + random() * (layer.speed_max - layer.speed_min),
...
local function update_snow(dt)
    ...
    for _, fl in ipairs(flakes) do
        ...
        if o and o.Parent then
            ...
            local px = o.Position.X.Offset
                + fl.drift
                + sin(fl.sway) * fl.sway_amp
                + _wind_current * wind_factor
...
function nebula:_rebuild_keybind_list()
    ...
    for _, entry in ipairs(nebula._keybind_entries) do
        ...
        local row = mk("Frame", {
            ...
            Size = dim2(1,-C.PAD_L-C.PAD_R,0,C.ROW_H), ZIndex = 1002,
        })
    ...
    if shown == 0 then
        ...
        body_h = C.PAD_TOP + shown * C.ROW_H + max(0, shown-1) * C.ROW_GAP + C.PAD_BOT
    end
...
function nebula:window(opts)
    ...
    local content = mk("Frame", {
        ...
        Position = dim2(0,0,0,58), Size = dim2(1,0,1,-58),
        ClipsDescendants = true,
    })
    ...
    if not nebula._blur then
        nebula._blur = ensure_blur()
    end
...
function nebula:toggle(opts)
    ...
    local function animate_toggle(state)
        if state then
            tween_svc:Create(track,
                TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                { BackgroundColor3 = theme.accent }
            ):Play()
            tween_svc:Create(knob,
                TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                {
    ...
    btn.MouseButton1Down:Connect(function()
        tween_svc:Create(knob,
            ...
            { Size = dim2(0, TOGGLE_KNOB_SIZE + 2, 0, TOGGLE_KNOB_SIZE - 1) }
        ):Play()
    end)
...
function nebula:dropdown(opts)
    ...
    local popup = mk("Frame", {
        ...
        Size = dim2(0,100,0,0), Visible = false, ZIndex = 200,
        ClipsDescendants = true,
    })
    ...
    local popup_target_h = 0

    local function calc_popup_h()
        local items_count = #cfg.items
        return clamp(items_count * 18 + 8, 0, 160)
    end
    ...
    function cfg.set_visible(bool)
        if bool then
            ...
            popup_target_h = calc_popup_h()
            popup.Size     = dim2(0, dd_btn.AbsoluteSize.X, 0, 0)
            popup.Visible  = true
            cfg.open       = true
            reposition_dd()

            tween_svc:Create(popup,
                TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                { Size = dim2(0, dd_btn.AbsoluteSize.X, 0, popup_target_h) }
            ):Play()
            ...
            closing_conn = uis.InputBegan:Connect(function(input, gp)
                ...
                local in_pop   = m.X>=pop_abs.X and m.X<=pop_abs.X+pop_size.X
                    and m.Y>=pop_abs.Y and m.Y<=pop_abs.Y+pop_size.Y
                local in_btn   = m.X>=btn_abs.X and m.X<=btn_abs.X+btn_size.X
            ...
            tween_svc:Create(popup,
                ...
                { Size = dim2(0, popup.Size.X.Offset, 0, 0) }
            ):Play()
        ...
        for _, item in ipairs(list) do
            ...
            opt.MouseButton1Click:Connect(function()
                if cfg.multi then
                    ...
                    if idx then remove(cfg.selected, idx)
                    else insert(cfg.selected, item) end
...
function nebula:textbox(opts)
    ...
    tb.Focused:Connect(function()
        tw(wrap_stroke, { Color = theme.accent, Thickness = 1.5 }, 0.15)
    end)
    tb.FocusLost:Connect(function()
        tw(wrap_stroke, { Color = theme.border, Thickness = 1 }, 0.15)
    end)
    ...
    function cfg.set_visible(bool)
        if bool then
            ...
            closing_conn = uis.InputBegan:Connect(function(input, gp)
                ...
                local p_size = picker.AbsoluteSize
                local s_abs  = swatch.AbsolutePosition
                local s_size = swatch.AbsoluteSize
                local in_p = m.X>=p_abs.X and m.X<=p_abs.X+p_size.X
                    and m.Y>=p_abs.Y and m.Y<=p_abs.Y+p_size.Y
                local in_s = m.X>=s_abs.X and m.X<=s_abs.X+s_size.X
...
function nebula:destroy()
    ...
    nebula._blur             = nil
    nebula._color_correction = nil
    nebula._dof              = nil
    nebula._snow             = nil
    nebula._snow_flakes      = nil
    nebula._vignette         = nil
    nebula._romania          = nil
